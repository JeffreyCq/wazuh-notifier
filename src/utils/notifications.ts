...
export async function sendAlertNotification(alert: Alert, clusterName: string): Promise<void> {
  const { rule, agent } = alert._source;
  await browser.notifications.create(alert._id, {
    type: 'basic',
    iconUrl: browser.runtime.getURL('icons/icon48.png'),
    title: `[${clusterName}] Level ${rule.level} Alert`,
...
export async function sendBatchNotification(
  ...
  await browser.notifications.create(`wazuh-batch-${Date.now()}`, {
    type: 'basic',
    iconUrl: browser.runtime.getURL('icons/icon48.png'),
    title: `[${clusterName}] ${count} New Critical Alerts`,
