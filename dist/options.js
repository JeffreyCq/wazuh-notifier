...
// EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js!./src/options/options.css
var options = __webpack_require__(974);
...
var update = injectStylesIntoStyleTag_default()(options/* default */.A, options_options);
       ...
       /* harmony default export */ const src_options_options = (options/* default */.A && options/* default */.A.locals ? options/* default */.A.locals : undefined);
...
async function saveConfig(config) {
    await browser_polyfill_default().storage.local.set({ config });
}
...
function readForm() {
    return {
        opensearchUrl: el('opensearch-url').value.trim(),
        username: el('username').value.trim(),
        password: el('password').value,
        pollIntervalMinutes: Math.max(1, parseInt(el('poll-interval').value, 10) || 2),
        minAlertLevel: parseInt(el('min-level').value, 10),
        enabled: el('enabled').checked,
    };
}
function populateForm(config) {
    el('opensearch-url').value = config.opensearchUrl;
    el('username').value = config.username;
    el('password').value = config.password;
    el('poll-interval').value = String(config.pollIntervalMinutes);
    el('min-level').value = String(config.minAlertLevel);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    el('min-level').addEventListener('input', (e) => {
        el('level-value').textContent = e.target.value;
    });
    // Show/hide password
    ...
    el('btn-save').addEventListener('click', async () => {
        ...
        await browser_polyfill_default().runtime.sendMessage({ type: 'CONFIG_UPDATED' });
