...
async function fetchCriticalAlerts(config, since) {
...
async function getSeenIds() {
    const result = await browser_polyfill_default().storage.local.get('seenIds');
...
async function addSeenIds(ids) {
    ...
    await browser_polyfill_default().storage.local.set({ seenIds: trimmed });
}
...
async function addRecentAlerts(alerts) {
    ...
    await browser_polyfill_default().storage.local.set({ recentAlerts: combined });
}
...
async function saveConnectionStatus(status) {
    await browser_polyfill_default().storage.local.set({ connectionStatus: status });
}
async function getLastPollTime() {
    const result = await browser_polyfill_default().storage.local.get('lastPollTime');
    return result['lastPollTime'] ?? 0;
}
async function saveLastPollTime(time) {
    await browser_polyfill_default().storage.local.set({ lastPollTime: time });
}
...
async function sendAlertNotification(alert) {
    ...
    await browser_polyfill_default().notifications.create(alert._id, {
        type: 'basic',
        iconUrl: browser_polyfill_default().runtime.getURL('icons/icon48.png'),
        title: `Wazuh — Level ${rule.level} Alert`,
        message: `${rule.description}\nAgent: ${agent.name} (${agent.id})`,
        priority: 2,
    });
}
async function sendBatchNotification(count, maxLevel) {
    await browser_polyfill_default().notifications.create(`wazuh-batch-${Date.now()}`, {
        type: 'basic',
        iconUrl: browser_polyfill_default().runtime.getURL('icons/icon48.png'),
...
async function poll() {
    ...
    if (!config.enabled || !config.opensearchUrl || !config.username)
        return;
    try {
        ...
        const stored = newAlerts.map((a) => ({ ...a, notifiedAt: Date.now() }));
        ...
        await browser_polyfill_default().action.setBadgeText({ text: badgeText });
        await browser_polyfill_default().action.setBadgeBackgroundColor({ color: '#ef4444' });
    }
    catch (err) {
...
async function setupAlarm(intervalMinutes) {
    await browser_polyfill_default().alarms.clear(ALARM_NAME);
    await browser_polyfill_default().alarms.create(ALARM_NAME, {
        periodInMinutes: Math.max(1, intervalMinutes),
        delayInMinutes: 0,
    });
}
browser_polyfill_default().alarms.onAlarm.addListener(async (alarm) => {
    if (alarm.name === ALARM_NAME)
        await poll();
});
browser_polyfill_default().runtime.onInstalled.addListener(async () => {
    const config = await getConfig();
    if (config.enabled)
        await setupAlarm(config.pollIntervalMinutes);
});
browser_polyfill_default().runtime.onStartup.addListener(async () => {
    const config = await getConfig();
    if (config.enabled)
        await setupAlarm(config.pollIntervalMinutes);
});
    ...
    switch (msg.type) {
        case 'CONFIG_UPDATED': {
            ...
            if (config.enabled) {
                ...
                await browser_polyfill_default().alarms.clear(ALARM_NAME);
                await browser_polyfill_default().action.setBadgeText({ text: '' });
            }
            break;
        }
        case 'POLL_NOW':
